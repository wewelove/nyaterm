...
WebDAV 认证同时支持 **Basic** 和 **Digest** 两种方式，当服务端要求 Digest 时 NyaTerm 会自动改用 Digest，以提升对强制 Digest 的 NAS / 网关的兼容性。
...
如果 provider 选的是 **Gitee 代码片段**，NyaTerm 会把加密后的可移植快照保存为一个 Gitee 代码片段（Gitee 代码片段 / 类 Gist）。对于身处 Gitee 访问较快、又不想自建 WebDAV / S3 的用户，这是一种比较省心的选择。

通常需要填写：

- **Gitee 个人访问令牌（Personal Access Token）**（需要代码片段 / gist 权限）
- **代码片段标识 / 描述**（可选，便于在 Gitee 上识别这条快照）

适合场景：

- 没有 NAS、私有云或对象存储，又希望快照存放在国内访问较快的位置
- 想用一个 Gitee 账号就完成跨设备同步，而不额外维护存储服务
