...
async fn active_session(
    ...
    let mut pending_graphics_region: Option<InclusiveRectangle> = None;
    let mut pending_graphics_deadline: Option<tokio::time::Instant> = None;
    ...
    let disconnect_reason = 'outer: loop {
        let outputs = tokio::select! {
            frame = reader.read_pdu() => {
                ...
                queue_stage_outputs(active_stage.process(&mut image, action, &payload)?)
            }
            input_event = input_event_receiver.recv() => {
                ...
                let stage_outputs = match input_event {
                ...
                queue_stage_outputs(stage_outputs)
            }
            ...
            _ = async { match fake_events_interval.as_mut() {
                ...
                let stage_outputs = if last_input.elapsed() >= fake_events_interval.as_ref().map_or(Duration::MAX, |i| i.period()) {
                    last_input = tokio::time::Instant::now();
            ...
            _ = async {
                match pending_graphics_deadline {
        ...
        for out in outputs {
            let Some(out) = out else {
                if let Some(region) = pending_graphics_region.take() {
                    pending_graphics_deadline = None;
                    send_image_patch(output_event_sender, &image, region).await?;
                }
                continue;
            };
            match out {
                ...
                ActiveStageOutput::GraphicsUpdate(region) => {
                    if pending_graphics_region.is_none() {
                        pending_graphics_deadline = Some(tokio::time::Instant::now() + Duration::from_millis(8));
                    }
                    pending_graphics_region = Some(match pending_graphics_region.take() {
...
fn queue_stage_outputs(outputs: Vec<ActiveStageOutput>) -> Vec<Option<ActiveStageOutput>> {
    outputs.into_iter().map(Some).collect()
}

async fn send_image_patch(
    output_event_sender: &mpsc::Sender<RdpOutputEvent>,
    image: &DecodedImage,
    region: InclusiveRectangle,
) -> SessionResult<()> {
    let buffer = image_patch_buffer(image, &region);
    output_event_sender
        .send(RdpOutputEvent::ImagePatch {
            buffer,
...
fn image_patch_buffer(image: &DecodedImage, region: &InclusiveRectangle) -> Vec<u32> {
    ...
    let patch_width = usize::from(region.width());
    let patch_height = usize::from(region.height());
    let row_bytes = patch_width * bytes_per_pixel;
    let mut buffer = Vec::with_capacity(patch_width * patch_height);
    let data = image.data();

    for row in 0..patch_height {
        let y = usize::from(region.top) + row;
        let start = y * source_stride + usize::from(region.left) * bytes_per_pixel;
        let end = start + row_bytes;
        buffer.extend(data[start..end].chunks_exact(4).map(|pixel| {
            let r = pixel[0];
            let g = pixel[1];
            let b = pixel[2];
